...
function App() {
  const [crrState, setState] = useState(0);
  const [crrCity, setCity] = useState(0);
  const [crrLand, setLand] = useState(0);
  ...
  return (
    <div id="main">
      ...
      {/* <div id="state-name">{states[crrState].name}</div> */}
      <div id="state-title">{states[crrState].name}</div>
      <div id="state-description">{states[crrState].description}</div>

      <select id="city" onChange={selCity}>
        {states[crrState].city.map((city, index) => {
          return <option value={index}>{city.name}</option>;
        })}
      </select>

      {/* <div id="city-name"></div> */}
      <div id="city-title">{states[crrState].city[crrCity].name}</div>
      <div id="city-description">
        {states[crrState].city[crrCity].description}
      </div>

      <select id="landmark" onChange={selLand}>
        {states[crrState].city[crrCity].landmarks.map((landmark, index) => {
          return <option value={index}>{landmark.name}</option>;
        })}
      </select>

      {/* <div id="landmark-name"></div> */}
      <div id="landmark-title">
        {states[crrState].city[crrCity].landmarks[crrLand].name}
      </div>
      <div id="landmark-description">
        {states[crrState].city[crrCity].landmarks[crrLand].description}
      </div>
    </div>
  );
}
